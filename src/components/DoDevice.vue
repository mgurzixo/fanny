      ...
      <q-card-section class="q-pt-none q-mb-sm">
        <div class="cursor-pointer" style="width: 100%">
          <q-form @submit="onSubmit" @reset="onReset" class="q-gutter-md">
            ...
            <q-input class="col-grow q-py-none q-my-none text-h5" rounded outlined v-model="newName" :rules="[
              (val) =>
                (val.length > 2 && val.length < 16) || 'Min 3, Max 15 characters',
              (val) =>
                /^([-_\.\p{L}\p{N}])+ ?$/u.test(val) ||
                'Invalid char. Letters, numbers or [-_.] only',
            ]">
            </q-input>
...
<script setup>
import { clearDevice } from "src/stores/fannyStore";

import { ref, onMounted, onUnmounted } from "vue";
import { mgToast, mgErrorToast } from "src/lib/mgToast";
...
async function doClose() {
  console.log(`[DoDevice.doClose] Closing '${props.device.id}'`);
  try {
    ...
    ble.disconnect(
      props.device.id,
      () => {
        console.log(`[DoDevice.doClose] disconnected OK`);
        isConnected.value = false;
        emit("disconnectEvent");
      },
      (e) => {
        console.log(`[DoDevice.doClose] Error disconnecting: ${e}`);
        isConnected.value = false;
        emit("disconnectEvent");
      }
    );
    ...
    console.log(`[DoDevice.doClose] e:${e}`);
    isConnected.value = false;
    emit("disconnectEvent");
  }
}
...
async function doBle(data, successCb, errorCb, timeoutMs) {
  console.log(`[DoDevice.doBle] sending data: '${data}'`);
...
function checkFanny() {
  ...
  doBle(stringToBytes(`V\n`), (data) => {
    ...
    if (!res) {
      console.log(`[DoDevice.checkFanny] Invalid device`);
      mgErrorToast("Invalid device\nDisconnecting.");
      doClose();
    } else {
      // console.log(`[DoDevice.checkFanny] res:${JSON.stringify(res)}`);
      fanName.value = props.device.fanName;
      fanVersion.value = res[1];
      speed.value = Number(res[3]) + 1;
