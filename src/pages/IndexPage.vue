      ...
      <div v-for="device in devices" :key="device.id">
        <q-item class="q-my-sm" clickable xv-ripple>
          <q-item-section>
            ...
            <q-item-label caption> Rssi:{{ device.rssi }} </q-item-label>
            <!-- <q-item-label caption>
              Lru:{{ device.lru }}
            </q-item-label> -->
            <q-item-label caption> Id:{{ device.id }} </q-item-label>
          </q-item-section>
...
import { isDialogOpen, devices, insertDevice, updateLru } from "src/stores/fannyStore";
...
function extSrvUUID(advertisementData) {
  // Complete 16-bit UUIDs, with esp32-c3
  try {
    var bytes = new Uint8Array(advertisementData["0x03"]);
    // console.log(`[IndexPage.extSrvUUID] ${JSON.stringify(bytes)}`);
    return bytes[1] * 256 + bytes[0];
  } catch (error) {
    return undefined;
  }
...
function scanOk(device) {
  ...
  // const local2 = localName.slice(0, 7); // myFanny)
  // if (local2 != "myFanny") {
  //   console.log(`[IndexPage.scanOk] (${local2}) Skipping ${localName} (${JSON.stringify(d)})`);
  //   return;
  // }
  ...
  console.log(`[IndexPage.scanOk] adv[1]:${bytesToString(d["0x01"])}`);

  let serviceUUID = srvUUID(d);
  if (!serviceUUID) {
    serviceUUID = extSrvUUID(d);
  }
  console.log(
    `[IndexPage.scanOk] device:${device.id} name:${device.name} localName:${localName} serviceUUID:${serviceUUID?.toString(16)}`
  );
